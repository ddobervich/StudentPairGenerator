...
public class Display {
    private static final float TEXT_SIZE = 45;
    ...
    public void drawGrid(Student[][] f) {
        String name;
        int numcols = f[0].length;
        int numrows = f.length;
        ...
        for (int i = 0; i < numrows; i++) {
            for (int j = 0; j < numcols; j++) {
                ...
                p.fill(0);
                p.text(name, x + j * dx, y + i * dy + dy/2);
            }
        }
    }
    ...
    public void highlightLocation(Location l, AssignmentGrid g) {
        if (g.isInGrid(l)) {
    ...
    public void initializeWithGrid(Student[][] grid) {
        if (grid == null) {
            System.out.println(
        ...
        System.out.println("Setting disply: # rows is " + grid.length
                + ", # cols is " + grid[0].length);
    }
}